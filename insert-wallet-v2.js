const fs = require('fs');
const filePath = 'C:/Users/Administrator/WorkBuddy/2026-05-05-task-1/klakna-backend/public/admin-app.js';

let content = fs.readFileSync(filePath, 'utf8');

// 找到 pgFinanceCoin 函数的结束位置（在 _go['finance-coin']=pgFinanceCoin; 之后）
const anchor = "_go['finance-coin']=pgFinanceCoin;";
const insertPos = content.indexOf(anchor) + anchor.length;

if (insertPos < anchor.length) {
...
const newContent = content.slice(0, insertPos) + newFunctions + content.slice(insertPos);

fs.writeFileSync(filePath, newContent, 'utf8');
console.log('Successfully inserted wallet config and collection functions!');
