...
function init() {
  ...
  wss.on('connection', (ws, req) => {
    ...
    if (ipCount >= MAX_CONNECTIONS_PER_IP) {
      console.log('[WS Control] ⚠️ IP 连接数超限:', clientIp, ipCount);
      ws.close(1008, 'Connection limit exceeded');
      return;
    }
    connectionsPerIp.set(clientIp, ipCount + 1);
    ws.__clientIp = clientIp; // 保存 IP 用于清理
    ...
    ws.on('close', () => {
      // 🛡️ 清理 IP 连接计数
      if (ws.__clientIp) {
        const count = connectionsPerIp.get(ws.__clientIp) || 0;
        if (count > 1) {
          connectionsPerIp.set(ws.__clientIp, count - 1);
        } else {
          connectionsPerIp.delete(ws.__clientIp);
        }
      }
    ...
    ws.on('error', (err) => {
      ...
      if (userId) {
        ...
        console.log('[WS Control] 错误断开:', userId, '| 在线:', getOnlineCount());
      }
    });
  });
